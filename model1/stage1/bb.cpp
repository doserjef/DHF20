...
int main(int argc, char **argv) {
  ...
  ***************************************************************************/
  ...
  double splineMin; par.getVal("spline.min", splineMin);
  double splineMax; par.getVal("spline.max", splineMax);
  
  double *y; int colY, n, a; par.getFile("y.file", y, n, colY);
  // double *Z; int i, j; par.getFile("Z.file", Z, i, j);
  double *x; int i, j; par.getFile("x.file", x, n, a); 
  ...
  ***************************************************************************/
  ...
  double mu, thetaCurr, logPostWCurr;
  ...
  double *thetaSamples, *wSamples, *fittedSamples, precision, *muSamples;
  ...
  ***************************************************************************/
  ...
  // gsl_bspline_knots_uniform(0.0, 7.0, bw);
  gsl_vector *knots = gsl_vector_alloc(nBreak);
  gsl_vector_set(knots, 0, splineMin);
  gsl_vector_set(knots, 1, 2);
  gsl_vector_set(knots, 2, splineMax);
  ...
  ***************************************************************************/
  ...
  for (a = 0, d = 0; a < nIter; ++a) {
    for (b = 0; b < nBatch; ++b, ++d) {
      ...
      ***********/
      ...
      // logPostThetaCurr += betaLogpostJacobianInd(n, y, x, eta, phi);
      ...
      for (c = 0; c < nTheta; ++c) {
        ...
        ***********/
        dgemv_(&transN, &n, &nSpline, &one, Z, &n, &theta[betaIndx], &inc, &zero,
               eta, &inc);
        daxpy_(&n, &one, w, &inc, eta, &inc);
        // logPostThetaCand += betaLogpostJacobianInd(n, y, x, eta, phi);
      ...
      ***********/
      ...
      // logPostWCurr += betaLogpostJacobianInd(n, y, x, tmp, phi);
      ...
      ***********************************/
      for (c = 0; c < n; ++c) {
        ...
        ***********/
        dcopy_(&n, eta, &inc, tmp, &inc);
        daxpy_(&n, &one, w, &inc, tmp, &inc);
        // logPostWCand += betaLogpostJacobianInd(n, y, x, tmp, phi);
  ...
  writeRMatrix(outFile + "-spline", Z, n, nSpline);
