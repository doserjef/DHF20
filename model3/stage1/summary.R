...
vals <- summary(window(mcmc(alpha.s), start=burn.in))
vals$quantiles

cov.samples <- alpha.s[, c("lambda.11", "lambda.21", "lambda.31", "lambda.22", "lambda.32", 
                          "lambda.33")]
...
png("../../../figures/fit-m3-stage1.png", width = 480, height = 480)
...
png("../../../figures/spline-fit-m3-stage1.png", width = 480, height = 480)
par(mar=c(6.5,4.1,4.1,2.1))
...
axis(1, las = 1)
axis(2, las = 1)
text(3.0, -0.27, "Model 3", xpd = TRUE, cex = 1.5)
dev.off()
...
# text(2.0, -0.27, "Model 2/3", xpd = TRUE, cex = 1.5)
# dev.off()
