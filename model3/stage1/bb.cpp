...
int main(int argc, char **argv) {
  ...
  ***************************************************************************/
  ...
  double threshold; par.getVal("threshold", threshold);
  ...
  ***************************************************************************/
  ...
  double logPostWCand, wCurr, acceptRate, *etaSamples;
  ...
  for (a = 0, d = 0; a < nIter; ++a) {
    for (b = 0; b < nBatch; ++b, ++d) {
      ...
      ***********/
      ...
      for (i = 0; i < nPhi; ++i) {
        logPostThetaCurr += iGammaLogpostJacobian(phi[i], phiA[i], phiB[i]);
      }
      ...
      ***********/
      ...
      logPostThetaCurr += betaLogpostJacobianInd(n, y, x, eta, phi, threshold);
      ...
      ***********************************/
      for (c = 0; c < nTheta; ++c) {
        ...
        ***********/
        ...
        for (i = 0; i < nPhi; ++i) {
          logPostThetaCand += iGammaLogpostJacobian(phi[i], phiA[i], phiB[i]);
        }
        ...
        ***********/
        dgemv_(&transN, &n, &nSpline, &one, Z, &n, &theta[betaIndx], &inc, &zero,
               eta, &inc);
        // daxpy_(&n, &one, w, &inc, eta, &inc);
        logPostThetaCand += betaLogpostJacobianInd(n, y, x, eta, phi, threshold);
      ...
      // logPostWCurr += betaLogpostJacobianInd(n, y, x, tmp, phi, threshold);
      // 
      // /************************************
      ...
      //   dcopy_(&n, eta, &inc, tmp, &inc);
      //   daxpy_(&n, &one, w, &inc, tmp, &inc);
      //   logPostWCand += betaLogpostJacobianInd(n, y, x, tmp, phi, threshold);
      // 
